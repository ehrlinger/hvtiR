# Getting Started with hvtiverse
...
`hvtiverse` reduces that to one call, and adds a way to ask whether you
are current.
...
that `hvtiverse` itself requires only R 4.1.0 — deliberately, so that
the diagnostic still runs on a machine whose R is too old for the
members it is reporting on.
